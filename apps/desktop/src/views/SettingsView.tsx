...
export function SettingsView(props: SettingsViewProps): JSX.Element {
  ...
  const doctorResults = useMemo(() => {
    ...
    if (staleSessions.length > 0) {
      issues.push({
        severity: 'warn',
        message: `${String(staleSessions.length)} stale session(s) — active but no update for >5 minutes`,
      });
    }
    ...
    if (orphanedRuns.length > 0) {
      issues.push({
        severity: 'warn',
        message: `${String(orphanedRuns.length)} orphaned run(s) — session no longer tracked`,
      });
    }
    ...
    if (offlineMachines.length > 0) {
      issues.push({
        severity: 'info',
        message: `${String(offlineMachines.length)} offline machine(s)`,
      });
    }
    ...
    if ((health?.failedRunsTotal ?? 0) > 0) {
      issues.push({
        severity: 'error',
        message: `${String(health?.failedRunsTotal ?? 0)} failed run(s) detected`,
      });
    }
    ...
    <section className="view-panel">
      ...
      <div className="settings-grid">
        <div className="settings-section">
          ...
          <div className="settings-row">
            <span className="settings-row-label">Auth Mode</span>
          ...
          {!snapshot ? (
            ...
            <div>
              <p className="doctor-hint">
                Run a quick diagnostic check to detect stale sessions, orphaned runs, and other anomalies.
              </p>
              <button className="btn-primary" onClick={() => { setDoctorRun(true); }}>
                Run Doctor
              </button>
            </div>
          ) : doctorResults ? (
            <div className="doctor-results">
              <div className="doctor-summary">
                <span className="doctor-stat">{String(doctorResults.totalSessions)} sessions</span>
                <span className="doctor-stat">{String(doctorResults.totalRuns)} runs</span>
                <span className="doctor-stat">{String(doctorResults.totalEvents)} events</span>
                <span className="doctor-stat">{String(doctorResults.totalLogs)} logs</span>
              </div>
